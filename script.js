...
document.addEventListener('keydown', event => {
  ...
  const modal =
    document.getElementById('benefitPreview');
...
function fallbackCopy(
  ...
  if (copied) {
    ...
    alert(
      '복사에 실패했습니다. 입력 내용을 직접 선택해 복사해주세요.'
    );
  }
}
...
(() => {
  ...
  function openLightbox(clickedImg) {
    const groupName =
      clickedImg.dataset.lightboxGroup ||
      'default';

    const clickedCard =
      clickedImg.closest('.work');

    const clickedCategory =
      clickedCard
        ? clickedCard.dataset.category
        : '';

    const showAllCategories =
      groupName === 'portfolio' &&
      isAllPortfolioTabActive();

    currentGroup =
      getZoomableImages().filter(img => {
        const sameGroup =
          (
            img.dataset.lightboxGroup ||
            'default'
          ) === groupName;
    ...
    currentIndex = Math.max(
      0,
      currentGroup.indexOf(clickedImg)
    );
    ...
    lightbox.classList.toggle(
      'gif-mode',
      isGifImage(clickedImg)
    );

    updateLightbox();

    const rect =
      clickedImg.getBoundingClientRect();
  ...
  function drawStaticGifThumbnail(
    canvas,
    src
  ) {
    const ctx =
...
(() => {
  ...
  function syncPortfolioDividers(
    button
  ) {
    const value = String(
